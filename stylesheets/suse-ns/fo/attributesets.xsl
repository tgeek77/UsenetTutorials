...
<xsl:attribute-set name="toc.line.properties">
  ...
  <xsl:attribute name="font-size">
    <xsl:choose>
      <xsl:when test="self::d:part"><xsl:value-of select="1.44 * $body.font.master"/>pt</xsl:when>
      <xsl:when test="self::d:chapter or 
                      self::d:preface or 
                      self::d:glossary or 
                      self::d:appendix"><xsl:value-of select="1.2 * $body.font.master"/>pt</xsl:when>
      <xsl:when test="self::d:sect1"><xsl:value-of select="$body.font.master"/>pt</xsl:when>
      <xsl:otherwise><xsl:value-of select="$body.font.master"/>pt</xsl:otherwise>
    </xsl:choose>
  </xsl:attribute>
  <xsl:attribute name="keep-with-next">
    <xsl:choose>
      <xsl:when test="self::d:part or
                      self::d:chapter or
                      self::d:preface or 
                      self::d:glossary or
                      self::d:appendix">always</xsl:when>
      <xsl:otherwise>auto</xsl:otherwise>
    </xsl:choose>
  </xsl:attribute>
  <xsl:attribute name="space-before">
    <xsl:choose>
      <xsl:when test="self::d:part">18pt</xsl:when>
      <xsl:when test="self::d:chapter or self::d:appendix or self::d:glossary">12pt</xsl:when>
      <xsl:otherwise><xsl:value-of select=".5 * $body.font.master"/>pt</xsl:otherwise>
    </xsl:choose>
  </xsl:attribute>
  <xsl:attribute name="font-weight">
    <xsl:choose>
      <xsl:when test="self::d:part">bold</xsl:when>
      <xsl:when test="self::d:chapter or 
                      self::d:preface or 
                      self::d:glossary or 
                      self::d:appendix">bold</xsl:when>
      <xsl:when test="self::d:sect1">normal</xsl:when>
      <xsl:otherwise>normal</xsl:otherwise>
    </xsl:choose>
  </xsl:attribute>
  <xsl:attribute name="font-family">
    <xsl:choose>
      <xsl:when test="self::d:part or self::d:chapter or self::d:appendix or
                      self::d:glossary or self::d:preface or self::d:reference">
...
<!--  <xsl:attribute name="font-family">
  ...
  <xsl:attribute name="color">
    <xsl:choose>
      ...
      <xsl:otherwise>
        <xsl:choose>
           <!-- <xsl:when test="self::d:xref">red</xsl:when> -->
           <xsl:when test="self::d:ulink">blue</xsl:when>
